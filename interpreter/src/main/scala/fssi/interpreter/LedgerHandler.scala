...
class LedgerHandler extends Ledger.Handler[Stack] {
  ...
  private def hashBlock(block: Block): Block = {
    val allBytes =  block.previousHash.bytes ++ block.height.toByteArray ++ block.transactions.foldLeft(
      Array.emptyByteArray)((acc, n) => acc ++ bytesToHashForTransaction(n)) ++ block.chainID.getBytes(
      "utf-8")
  ...
  private def bytesToHashForTransaction(transaction: Transaction): Array[Byte] = transaction match {
    ...
    case x: Transaction.PublishContract =>
      ...
      x.contract.meta.methods.foldLeft(Array.emptyByteArray)((acc, n) => acc ++ n.toString.getBytes("utf-8")) ++
