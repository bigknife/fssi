...
trait MessageReceiver {
  private val applicationReceiver                                      = Executors.newSingleThreadExecutor()
  private val nominationReceiver                                       = Executors.newSingleThreadExecutor()
  private val consensusReceiver                                        = Executors.newSingleThreadExecutor()
  ...
  def receive(msg: Message): Unit = {
    msg match {
      case x: ApplicationMessage =>
        applicationReceiver.submit(new Runnable {
      ...
      case x: SCPEnvelope
        ...
        nominationReceiver.submit(new Runnable {
          override def run(): Unit = {
      ...
      case x: SCPEnvelope =>
        consensusReceiver.submit(new Runnable {
          override def run(): Unit = {
            logTimeCost(s"[RECV] ${x.messageType}@${x.slotIndex} message " +
                          s"from ${x.value.statement.from}",
                        "    pushed into message pool, ") {
